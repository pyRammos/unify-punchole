...
def update_ip():
    ...
    override_ip = data.get('override_ip')
    ...
    # === Step 1: Use override IP if provided ===
    if override_ip:
        ip = override_ip
        print(f" 📡 IP overridden by client payload: {ip}")
    else:
        # === Step 2: Extract IP from headers ===
        ...
        if not ip:
            ...
            if xff:
                ip_candidates = [ip.strip() for ip in xff.split(",")]
        ...
        print(f" 📡 Raw IP extracted from headers: {ip}")
